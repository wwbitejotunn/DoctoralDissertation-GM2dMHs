...
为了进一步验证在西面体方法中计算所得的片面处的\cemb{Sb}原子的赝氢原子的赝化学势$\tmuVar{H\left(Sb\right)}{face}$是否能够较好的对切片模型中钝化底部原子赝氢的$\tmuVar{H\left(Sb\right)}{}$进行近似，我们构造了一个上下表面均被赝氢钝化的\cemb{InSb}切片模型。在这样的切片模型中，上下表面赝氢的赝化学势之和可以写成如下的形式\chinesecolon
\[
