...
\end{equation}
其中，净化学反应计量数$v_{ki}$为正负反应计量数之差$v_{ki}=v'_ki-v''_ki$；$q_i$为第$i$个化学反应的反应进度。对于一个化学反应，其反应进度可以用正负反应率之差进行表示\chinesecolon
\begin{equation}
    \label{eq:CHEM_rateOfProgress}
    q_i=k'_i\prod_{k=1}^{K}\left[X_k\right]^{v'_{ki}}-k''_i\prod_{k=1}^{K}\left[X_k\right]^{v''_{ki}}
\end{equation}

在式\ref{eq:CHEM_rateOfProgress}中，$\left[X_k\right]$为第$k$个化合物的浓度。$k'_i$和$k'’_i$为第$i$个反应的正负反应速率常数。
